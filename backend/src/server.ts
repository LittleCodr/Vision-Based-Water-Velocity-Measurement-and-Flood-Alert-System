...
import { authAdmin, db, storage } from './firebase.js';
...
app.post('/api/auth/register', async (req, res) => {
  ...
  if (!email || !password || !apiKey) return res.status(400).json({ message: 'Missing fields or API key' });
  try {
...
app.post('/api/auth/role', requireAuth, async (req: AuthedRequest, res) => {
  const uid = req.user?.uid;
  if (!uid) return res.status(401).json({ message: 'Unauthenticated' });
  const { role } = req.body;
  if (!role) return res.status(400).json({ message: 'Role required' });
  await db.collection('users').doc(uid).set(
    { email: req.user?.email, role, updatedAt: new Date().toISOString() },
    { merge: true }
  );
...
// Video upload
app.post('/api/upload/video', requireAuth, upload.single('file'), async (req: AuthedRequest, res) => {
  const file = req.file;
...
// Dataset upload
app.post('/api/upload/dataset', requireAuth, upload.single('file'), async (req: AuthedRequest, res) => {
  const file = req.file;
  if (!file) return res.status(400).json({ message: 'File required' });
  try {
    const id = uuid();
...
// Velocity logs
app.post('/api/velocity', requireAuth, async (req: AuthedRequest, res) => {
  const { velocity, source = 'sensor', timestamp } = req.body;
  if (velocity === undefined) return res.status(400).json({ message: 'Velocity required' });
  try {
    const id = uuid();
    const doc = {
      id,
...
// Alerts
app.post('/api/alerts', requireAuth, async (req: AuthedRequest, res) => {
  const { threshold, velocity, status } = req.body;
  if (threshold === undefined || velocity === undefined || !status) return res.status(400).json({ message: 'Missing fields' });
  try {
    const id = uuid();
    const doc = {
      id,
...
app.post('/api/inference/velocity', requireAuth, async (req: AuthedRequest, res) => {
  const { velocity, source = 'sensor' } = req.body;
  if (velocity === undefined) return res.status(400).json({ message: 'Velocity required' });
  const v = Number(velocity);
  if (Number.isNaN(v)) return res.status(400).json({ message: 'Velocity must be numeric' });

  const warn = velocityAlertThreshold || 2.5;
  const danger = defaultDanger || warn * 1.5;
  let label: 'normal' | 'warning' | 'danger' = 'normal';
  if (v >= danger) label = 'danger';
  else if (v >= warn) label = 'warning';
